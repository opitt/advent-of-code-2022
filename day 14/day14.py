    ...
    for y in range(h):
        ...
        for x in range(min(cave), max(cave) + 1):
            if y not in cave.get(x, []):
                ...
                # c = cave[x][y]
                row += f"[blue on yellow]{cave[x][y]}[/blue on yellow]"
        print(f"{y:>2}{row}")
    print("")
...
def solve1(cave):
    def drop(x, sandy):
        ...
        nexty = groundy + 1
        if cave[left].get(nexty, None) == None:
            return drop(left, groundy)
        if cave[right].get(nexty, None) == None:
            return drop(right, groundy)
        cave[x][groundy] = "o"  # land the sand
        return True
        ...
        for p1, p2 in it.pairwise(points):
            ...
            for y in range(min(y1, y2), max(y1, y2) + 1):
                for x in range(min(x1, x2), max(x1, x2) + 1):
                    stones = cave.get(x, dict())
                    if stones.get(x, None) == None:
                        stones[y] = "#"
                    cave[x] = stones
    return cave
